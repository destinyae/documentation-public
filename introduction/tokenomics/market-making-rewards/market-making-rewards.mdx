...
3. Orderly will actively manage the list of eligible market makers to participate in the market making rewards program, assessed based on their market making performance.
   ...
   2. Market makers that have underperformed can be dropped from the eligibility list. Similarly, market makers that are currently not on the eligibility list but have performed well can be invited by the Orderly team to join the program.
...
Market making rewards' market pool multiplier, the minimum depth, and the max spread of each market symbol are detailed below. These tables will be periodically updated.
...
When determining best bid/ask price, Orderly considers the sum of the bid order(s) / ask order(s) amount at a price level that has met the minimum depth requirement.
